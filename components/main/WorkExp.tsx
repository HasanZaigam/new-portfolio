...
const ExperienceTimeline = () => {
  return (
    <div className="relative py-16 px-6 sm:px-10 flex flex-col items-center bg-[#060606] text-white">
      {/* Heading */}
      <h2 className="text-4xl sm:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-cyan-500 mb-16">
        Work Experience
      </h2>

      {/* Timeline Line for Desktop */}
      <div className="hidden md:block absolute w-[3px] left-1/2 transform -translate-x-1/2 bg-gradient-to-b from-purple-500 to-cyan-500 h-full blur-[1px] opacity-70 animate-pulse"></div>

      <div className="w-full max-w-5xl space-y-12">
        {experiences.map((exp, index) => (
          <motion.div
            key={exp.id}
            className="relative flex flex-col md:flex-row items-center w-full space-y-6 md:space-y-0"
            variants={index % 2 === 0 ? slideInFromLeft(0.3) : slideInFromRight(0.3)}
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
          >
            {/* Neon Card */}
            <div
              className="relative w-full md:w-5/12 p-6 rounded-xl bg-[#121212] border border-purple-500/50 shadow-lg shadow-cyan-500/20 transition-all hover:scale-[1.05]"
            >
              {/* Job Type Badge */}
              <div className="absolute top-[-16px] left-4 bg-black/80 text-white px-4 py-1 text-xs sm:text-sm font-bold rounded-full border border-purple-500">
                {exp.type}
              </div>
              ...
              <div className="flex items-center mb-4">
                <Image
                  src={exp.logo}
                  alt={exp.company}
                  className="w-12 h-12 sm:w-14 sm:h-14 mr-4 rounded-lg shadow-md object-contain border-2 border-cyan-500"
                />
                <div>
              ...
              <p className="text-gray-400 text-xs sm:text-sm mb-4">{exp.location}</p>

              <p className="text-xs sm:text-sm text-gray-200 leading-relaxed">{exp.description}</p>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <button
        ...
        className="mt-12 px-6 py-2 text-white bg-purple-600 hover:bg-purple-500 rounded-lg font-medium shadow-md"
      >
        Contact Me
      </button>
    </div>
